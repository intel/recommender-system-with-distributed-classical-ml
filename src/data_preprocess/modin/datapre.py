...
def categorify(cols, df, dict_dfs, doSplit=False, sep='\t'):
    ...
    for i in cols:
        ...
        if doSplit:
            ...
            df[col_name] = df[src_name].apply(lambda x: indexing[x])
...
def model_merge(df, te_train_dfs):
    ...
    for te_train_df in te_train_dfs:
        ...
        df = pd.merge(df, dict_df, on=keys)
...
def valid_mergeFeatures(df, te_test_dfs, y_mean_all_df, output_name, mode, dict_dfs=None):
    ...
    if mode == "stage1":
        df.to_parquet(output_name+'/stage1_valid')
    elif mode == "stage2":
        df.to_parqut(output_name+'/stage2_valid')
    else:
        raise NotImplementedError("mode need to be stage1 or stage2")
...
def train():
    ...
    original_folder = '/sample2/sample_data.parquet'
...
def valid_stage1():
    ...
    val_df = valid_mergeFeatures(df, te_test_dfs, y_mean_all_df, output_name="stage1_valid",mode="stage1", dict_dfs=dict_dfs)
