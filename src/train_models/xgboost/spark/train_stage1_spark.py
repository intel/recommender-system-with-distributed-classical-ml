...
if __name__ == "__main__":
    ...
    train_data, valid_data = read_data(path_prefix+hdfs_data_path+'/stage1')
    
    valid = valid_data.select(col("tweet_id"), col("engaging_user_id"))
    ...
    for i in range(4):
        ...
        valid = valid.join(eval_results.select(col("tweet_id"), col("engaging_user_id"),col(f"pred_{name}")), on=['tweet_id','engaging_user_id'], how='left')

        print("saving models...")
        xgb_clf_model.write().overwrite().save(path_prefix+f"{hdfs_model_path}/xgboost_{name}_stage1.model")
        print("training done...")

    valid.repartition(1).write.option("header",True).mode("overwrite").csv(path_prefix + hdfs_pred_path + f"/xgboost_pred_stage1.csv")
