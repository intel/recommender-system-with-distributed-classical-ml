...
if __name__ == "__main__":
    ...
    for numlabel in range(1):
        ...
        def train_and_evaluate_model(run):
            param = dict(
                ...
                subsample        = run.params.subsample,
                colsample_bytree = run.params.colsample_bytree,
        ...
        experiment_config = dict(
            name="RecSys Vanilla SigOpt using the xgboost API",
            parameters=[
                ...
                dict(name="gamma", bounds=dict(min=0,max=5), type="int"),
                dict(name="reg_alpha", bounds=dict(min=0, max=6), type="double"),
                dict(name="reg_lambda", bounds=dict(min=0, max=2), type="double")
                ],
            metrics=[
                dict(name="ap", objective="maximize", strategy="optimize")
                ],
            budget = 30,
        )         
        ...
        for run in experiment.loop():
            with run:
                ap = train_and_evaluate_model(run)
                run.log_metric("ap", ap)
